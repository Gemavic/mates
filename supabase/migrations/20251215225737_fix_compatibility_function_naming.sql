/*
  # Fix Compatibility Function Parameter Naming

  Fixes naming conflicts between function parameters and table columns.
*/
...
BEGIN
  SELECT * INTO seeker_profile FROM public.user_profiles WHERE user_id = p_seeker_id;
  SELECT * INTO match_profile FROM public.user_profiles WHERE user_id = p_potential_match_id;
  SELECT * INTO seeker_prefs FROM public.matching_preferences WHERE user_id = p_seeker_id;
  ...
  IF seeker_profile.geo_location IS NOT NULL AND match_profile.geo_location IS NOT NULL THEN
    distance_km := calculate_distance_km(p_seeker_id, p_potential_match_id);
  ...
  WHERE ms.user_id = p_seeker_id 
    AND ms.potential_match_id = p_potential_match_id
...
COMMENT ON FUNCTION calculate_compatibility IS 'Calculate compatibility score (0-100) between two users';
