...
-- Note: Using ALTER TABLE with safe alterations
...
DECLARE
  ...
  error_message TEXT;
BEGIN
  -- Log the start of user profile creation (for debugging)
  ...
  -- Create user profile automatically with error handling
  BEGIN
    INSERT INTO public.user_profiles (
      ...
      'not_started', -- Default verification status
      false, -- Not online by default
      'public', -- Public profile by default
      true, -- Show online status by default
      '', -- Empty bio
      '{}', -- Empty interests array
      'serious', -- Default looking_for
      50, -- Default distance preference
      18, -- Min age
      99, -- Max age
      NOW(),
      NOW()
  ...
  -- Create user credits record with error handling
  BEGIN
...
EXCEPTION WHEN OTHERS THEN
  ...
  RETURN NEW; -- Still return NEW to not break the auth.users insert
END;
$$;
